...
proc run_tcl_script {args} {
    ...
    if { $tool == "openroad" } {
        ...
        if { [info exists ::env(SYNTH_ENB_CG)] && $::env(SYNTH_ENB_CG) } {
            #Generate clock gating yosys plugin - Dinesh A
            puts_info "Generating Synthesis clock gating plugin ... "
            set exit_code [catch {exec yosys-config --build $::env(synthesis_tmpfiles)/cg_plugin.so $::env(OPENLANE_ROOT)/plugin/yosys/Lighter/clock_gating_plugin.cc} error_msg]

            puts_info "Synthesis Running with Clock Gating Enabled ... "
            set args "$::env(SYNTH_BIN) -c $script -m $::env(synthesis_tmpfiles)/cg_plugin.so |& tee $::env(TERMINAL_OUTPUT) $arg_values(-indexed_log)"
        } else {
