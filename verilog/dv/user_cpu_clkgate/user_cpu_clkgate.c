...
int main()
{
   ...
   reg_glbl_mail_box = 0x1 << (bthread_get_core_id() * 8); // Start of Main 
    ...
    reg_cpu_clk_cfg |= 0x0000020; // CPU Wavekup based on exter interrupt 
    reg_timer_glbl_cfg = 39;
    reg_timer_cfg_timer0 = 0x10008;
    reg_glbl_intr_msk = 0x07;

    while(1) {

       reg_cpu_clk_cfg |= 0x01000020;  // Goto Risv Sleep
       reg_glbl_intr = 0x07; // Clear Timer Interrupt
